    ...
    while(ch!=2){
		...
		switch(ch){
			case 1:
				...
				system("cls");
				printBoard(arr, n);
				//checks the player win indicator
				if(P==1){
					SetColor(12);   		
					printf("\n\t\t%s Wins!!\n",player1);
					SetColor(15);   		
...
//initialize the strucure variables and links them to each other
//also intializes value of data to " "
void initializeBoard(struct node *arr[], int n) {
    int i, j;
